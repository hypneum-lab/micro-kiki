...
# ///
"""
Ralph Loop Runner — v0.2 quantum-inspired.

Executes AI iterations until all v0.2 stories complete.
Uses rich for beautiful terminal output with progress tracking.
Reads from `.ralph-v0.2/` instead of `.ralph/` to avoid colliding with the v0.1 loop.
"""
...
def run_claude() -> int:
    ...
    result = subprocess.run([
        ...
        "--print",
        "Execute ralph loop iteration per .ralph-v0.2/CLAUDE.md"
    ])
...
def show_header(max_iterations: int, complete: int, total: int):
    ...
    console.print(Panel.fit(
        f"[bold cyan]Ralph Loop Runner — v0.2 quantum-inspired[/]\n\n"
...
def show_completion():
    ...
    console.print()
    console.print(Panel.fit(
        "[bold green]All v0.2 stories complete![/]",
        border_style="green"
    ))
...
def main():
    ...
    if not RALPH_DIR.exists():
        console.print("[red]Error:[/] .ralph-v0.2 directory not found")
        console.print("[dim]Run from the micro-kiki repo root on branch quantum-inspired[/]")
        sys.exit(1)
