...
class _MLXRuntimeAdapter:
    ...
    * ``generate`` uses ``mlx_lm.generate`` with
    ...
    def __init__(
        ...
        # LRU prompt cache: reuses KV states across requests sharing a
        # common prefix (system prompt, repeated context).  The K candidates
        # generated by the Negotiator share the exact same prompt, so the
        # 2nd and 3rd candidates skip the entire prefill phase.
        from mlx_lm.models.cache import LRUPromptCache

        self._prompt_cache = LRUPromptCache(max_size=8)
        log.info("LRU prompt cache initialised (max_size=8)")
