...
## Purpose
All declarative configuration for micro-kiki. Two layers: (1) MLX LoRA training configs — one per domain plus curriculum batches and a rank-32 foundations variant; (2) runtime configs for the cognitive stack — the 35-domain -> 7 meta-intent map, capability flags (web search, self-critique levels, deep eval), search backend credentials/caches, and `micro_kiki/` brainstacks + domain definitions consumed by the data pipeline. No code; everything here is loaded by `src/routing`, `src/orchestrator`, `src/search`, and the MLX trainers.

## Key Files
| File | Description |
|------|-------------|
| `meta_intents.yaml` | Domain indices 0-34 -> 7 meta-intents (quick-reply, reasoning, coding, creative, research, agentic, tool-use) |
| `capabilities.yaml` | 5 capability outputs (web_search, self_critique_{token,response,task}, deep_eval) served alongside the 35-way router head, per-capability thresholds, best-of-N confidence bands, agentic loop max iterations |
...
| `stack-01-chat-fr.yaml` ... `stack-35-<last>.yaml` | One per-domain LoRA config (rank tier depends on domain per `CLAUDE.md`, alpha = 2*rank, scale 2.0, attention-only) |

## Subdirectories
| Directory | Purpose |
|-----------|---------|
| `micro_kiki/` | Brainstacks + domain definitions for the data pipeline (`brainstacks.yaml`, `domains.yaml`) — 35 domains with keywords, regex patterns, teachers, existing HF sources |
...
| `mlx-per-domain/` | Per-domain MLX configs for all 35 stacks (chat-fr, reasoning, python, typescript, cpp, rust, html-css, shell, sql, yaml-json, docker, kicad-dsl, spice, lua-upy, embedded, stm32, iot, freecad, platformio, power, emc, dsp, electronics, kicad-pcb, web-frontend, web-backend, music-audio, devops, llm-orch, math, security, components, llm-ops, ml-training) |
...
- `meta_intents.yaml` — each of the 35 domains must appear in exactly ONE intent bucket (the file enforces this by construction).
...
- `tests/routing/test_router_37.py` validates the legacy 32+5 capability shape (kept for migration).
